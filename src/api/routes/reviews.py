...
from src.db.models import Content, Review, ReviewLike
...
def _review_to_response(review: Review, like_count: int) -> ReviewResponse:
    # [수정] model_dump에서 like_count를 제외하여 인자 중복 방지
    return ReviewResponse(
        **review.model_dump(exclude={"like_count"}),
...
@router.get(
    "/reviews/popular",
    responses={
        **success_example(description="인기 리뷰 목록 조회"),
        400: error_example(400, ErrorCode.INVALID_QUERY_PARAM, "잘못된 요청입니다."),
    }
)
...
@router.post(
    ...
    responses={
        **success_example(ReviewResponse, status_code=201, message="리뷰가 등록되었습니다."),
        400: error_example(400, ErrorCode.VALIDATION_FAILED, "입력값이 유효하지 않습니다."),
        401: error_example(401, ErrorCode.UNAUTHORIZED, "로그인이 필요합니다."),
        404: error_example(404, ErrorCode.RESOURCE_NOT_FOUND, "콘텐츠를 찾을 수 없습니다."),
        409: error_example(409, ErrorCode.DUPLICATE_RESOURCE, "이미 리뷰를 작성했습니다."),
...
def create_review(
    ...
    if existing: 
        raise http_error(409, ErrorCode.DUPLICATE_RESOURCE, "이미 해당 콘텐츠에 리뷰를 작성했습니다.")
...
@router.get(
    ...
    responses={
        **success_example(ReviewListResponse, message="리뷰 목록이 조회되었습니다."),
        400: error_example(400, ErrorCode.INVALID_QUERY_PARAM, "정렬 파라미터 오류"),
        404: error_example(404, ErrorCode.RESOURCE_NOT_FOUND, "콘텐츠를 찾을 수 없습니다."),
    }
)
def get_reviews_by_content(
    ...
    stmt = (
        select(Review, like_counts.c.like_count)
        .join(like_counts, like_counts.c.id == Review.id, isouter=True) # isouter=True 추가하여 좋아요 없는 리뷰도 조회
...
@router.put(
    ...
    responses={
        **success_example(ReviewResponse, message="리뷰가 수정되었습니다."),
        400: error_example(400, ErrorCode.VALIDATION_FAILED, "입력값이 유효하지 않습니다."),
        401: error_example(401, ErrorCode.UNAUTHORIZED, "로그인이 필요합니다."),
        403: error_example(403, ErrorCode.FORBIDDEN, "자신이 작성한 리뷰만 수정할 수 있습니다."),
